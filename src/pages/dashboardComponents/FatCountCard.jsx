...
const Fat_API_URL = `${API_BASE_URL}/n_fat_splitter`;



/**

 * Main Stat Card 1 (Top-Left-Top) - Counts the total number of Fat.

 */

export default function FatCountCard() {
  ...
  useEffect(() => {
    ...
    const fetchFatCount = async () => {
      ...
      try {
        ...
        const response = await fetch(Fat_API_URL);
        ...
        console.error("Error fetching Fat data:", e);

        setError("Failed to load joint data. Check network/API.");

        setFatCount(0); // Reset count on error
  ...
  return (
    ...
    <div style={cardStyle}>
        ...
        <div style={titleContainerStyle}>
          ...
          <div style={titleStyle}>Total FAT Splitters</div>
        ...
        {!isLoading && !error && (
          ...
          <div style={countStyle}>{FatCount.toLocaleString()}</div>
