...
const JOINTS_API_URL = `${API_BASE_URL}/n_fdtPon`;



/**

 * Main Stat Card 1 (Top-Left-Top) - Counts the total number of Joints.

 */

export default function JointsCountCard() {
  ...
  useEffect(() => {
    ...
    const fetchJointsCount = async () => {
      ...
      try {
        ...
        const response = await fetch(JOINTS_API_URL);
        ...
        console.error("Error fetching joints data:", e);

        setError("Failed to load joint data. Check network/API.");

        setJointsCount(0); // Reset count on error
  ...
  return (
    ...
    <div style={cardStyle}>
        ...
        <div style={titleContainerStyle}>
          ...
          <div style={titleStyle}>Total FDTs</div>
        ...
        {!isLoading && !error && (
          ...
          <div style={countStyle}>{jointsCount.toLocaleString()}</div>
