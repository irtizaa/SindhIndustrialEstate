...
import RawalpindiMapWidgets from "./RawalpindiMapWidgets";
...
const parsePoles = (p) => {
  ...
  return {
    ...
    fetchData("http://localhost:5000/api/r_handhole", setHandholes, parseCoordinates);
    fetchData("http://localhost:5000/api/r_joints", setJoints, parseCoordinates);
    fetchData("http://localhost:5000/api/r_customers", setCustomers, parseCoordinates);
    fetchData("http://localhost:5000/api/r_poles", setPoles, parsePoles);

    fetchData("http://localhost:5000/api/r_fat", setFat, parseFat);
    fetchData("http://localhost:5000/api/R_fdt", setFdt, parseFdt);

    fetchData("http://localhost:5000/api/r_nodes", setNodes, parseCoordinates);
    fetchData("http://localhost:5000/api/r_metroFiber", setFibers, parseFiber);
    fetchData("http://localhost:5000/api/r_fa", setFiberAttachments, parseFiberAttachment);
  }, []);

  // Map Initialization
  useEffect(() => {
    loadGoogleMapsScript(() => {
      const Rawalpindi = { lat: 33.5848, lng:  73.0658 };
  ...
  return (
    <MainLayout>
      <div style={{ height: "78vh", width: "100%" }} ref={mapRef}></div>
       ...
       {/* ✅ External Layer List (independent of toolbox widget) */}
    <LayerListPanel
