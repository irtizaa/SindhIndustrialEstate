...
import KharianMapWidgets from "./KharianMapWidgets";
...
const parsePoles = (p) => {
  ...
  return {
    ...
    fetchData("http://localhost:5000/api/ka_handhole", setHandholes, parseCoordinates);
    fetchData("http://localhost:5000/api/ka_joints", setJoints, parseCoordinates);
    fetchData("http://localhost:5000/api/ka_customers", setCustomers, parseCoordinates);
    fetchData("http://localhost:5000/api/Ka_poles", setPoles, parsePoles);

    fetchData("http://localhost:5000/api/ka_fat_splitter", setFat, parseFat);
    fetchData("http://localhost:5000/api/ka_fdtPon", setFdt, parseFdt);

    fetchData("http://localhost:5000/api/ka_nodes", setNodes, parseCoordinates);
    fetchData("http://localhost:5000/api/ka_metroFiber", setFibers, parseFiber);
    fetchData("http://localhost:5000/api/ka_fiberattachment", setFiberAttachments, parseFiberAttachment);
  }, []);

  // Map Initialization
 useEffect(() => {
    loadGoogleMapsScript(() => {
      const Rawalpindi = { lat: 32.8143, lng:  73.8831 };  
        ...
        marker.addListener("click", () => {
          const html = buildInfoCard({
            ...
            list: [
              ...
              { icon: "🏢", label: "OLT_ID", value: c.OLT_ID },
              ...
              { icon: "🏢", label: "Ring_Name", value: c.Ring_Name },
              { icon: "🏢", label: "City", value: c.City },
