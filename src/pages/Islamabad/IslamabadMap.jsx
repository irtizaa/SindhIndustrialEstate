...
import LahoreMapWidgets from "./LahoreMapWidgets";
...
const parseFdt = (f) => {
  ...
  return {
    ...
    fetchData("http://localhost:5000/api/l_handhole", setHandholes, parseCoordinates);
    fetchData("http://localhost:5000/api/l_joints", setJoints, parseCoordinates);
    fetchData("http://localhost:5000/api/l_customers", setCustomers, parseCoordinates);

    fetchData("http://localhost:5000/api/l_fat", setFat, parseFat);
    fetchData("http://localhost:5000/api/l_fdt", setFdt, parseFdt);

    fetchData("http://localhost:5000/api/l_nodes", setNodes, parseCoordinates);
    fetchData("http://localhost:5000/api/l_metroFiber", setFibers, parseFiber);
    fetchData("http://localhost:5000/api/l_fa", setFiberAttachments, parseFiberAttachment);
  }, []);

  // Map Initialization
  useEffect(() => {
    loadGoogleMapsScript(() => {
      const Rawalpindi = { lat: 33.6996, lng: 73.0362 };  
  ...
  useEffect(() => {
    ...
    [...handholeMarkersRef.current, ...jointsMarkersRef.current, ...customersMarkersRef.current, ...nodesMarkersRef.current, ...fatMarkersRef.current, ...fdtMarkersRef.current].forEach(m => m.setMap(null));
        ...
        marker.addListener("click", () => {
          const html = buildInfoCard({
            ...
            list: [
              ...
              { icon: "🏢", label: "Business Name", value: c.Business_Name },
...
if (showFdt) {
  const markers = fdt.map(f => {
    const marker = new window.google.maps.Marker({
      ...
      icon: {
        url: fdtIcon, // you can change icon if needed
        scaledSize: new window.google.maps.Size(20, 20),
      },
    });
    ...
    marker.addListener("click", () => {
      showLeftPanel(
        buildInfoCard({
          ...
          tag: "FDT",
          image: f.Image_URL || "https://placehold.co/280x160/FF9800/FFFFFF?text=FDT",
          list: [
 ...
 polyline.addListener("click", (e) => {
  ...
  }, [map, handholes, joints, customers, nodes,fat,fdt, fibers, fiberAttachments, showHandholes, showJoints, showCustomers, showNodes,showFat, showFdt, showFibers, showFiberAttachments]);
